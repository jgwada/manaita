...
// ===MANUAL_START=== と ===MANUAL_END=== の間を抽出
...
function ManualBlock({ text, shopName, manualType }: { text: string; shopName: string; manualType: string }) {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      // A4サイズ（mm）で作成、画像をページ幅に合わせてスライス
      const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
      const pageW = 210
      const pageH = 297
      // mm単位での画像高さ（画像幅=595px相当を210mmにマッピング）
      const imgHeightMm = (img.height / img.width) * pageW * (1 / 2) // pixelRatio=2なので÷2
      ...
      <div className="bg-[#111008] px-4 py-3 flex items-center justify-between gap-2">
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <button
            ...
            disabled={exporting}
            className="text-xs px-3 py-1.5 rounded-lg font-medium bg-[#E8320A] text-white hover:bg-[#c92b09] transition-colors disabled:opacity-50"
          >
            {exporting ? '生成中…' : 'PDF'}
          </button>
        </div>
      ...
      <div style={{ position: 'fixed', top: 0, left: '-9999px', zIndex: -1 }}>
        ...
        >
          ...
          <div style={{ backgroundColor: '#111008', padding: '28px 36px 24px' }}>
            <div style={{ fontSize: '10px', color: '#9A8880', letterSpacing: '0.1em', marginBottom: '6px' }}>
              STAFF MANUAL
            </div>
            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#FFFFFF', lineHeight: 1.2 }}>
              {shopName}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
              <div style={{ width: '24px', height: '2px', backgroundColor: '#E8320A' }} />
              <div style={{ fontSize: '13px', color: '#E8320A', fontWeight: 'bold' }}>
                {manualType}マニュアル
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: '28px 36px 40px' }}>
            <ManualLines lines={lines} forPdf />
          </div>

          {/* PDFフッター */}
          <div style={{
            borderTop: '1px solid #EDE5DF',
            padding: '12px 36px',
            display: 'flex',
...
function MessageBubble({ message, shopProfile, manualType }: {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-full bg-[#E8320A] flex items-center justify-center text-white text-xs font-bold mr-2 flex-shrink-0 mt-1">
          AI
        </div>
      )}
      <div className={`max-w-[82%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
        ...
        {message.content}
        {message.isStreaming && (
          <span className="inline-block w-1.5 h-4 bg-[#E8320A] ml-1 animate-pulse rounded-sm" />
        )}
      </div>
    </div>
  )
...
export default function ManualPage() {
  ...
  const sendMessage = async (userContent?: string) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m =>
        m.id === assistantId
          ? { ...m, content: '応答に失敗しました。もう一度お試しください。', isStreaming: false }
          : m
      ))
    } finally {
      setIsLoading(false)
      ...
      <AuthGuard>
        <div className="min-h-screen bg-[#FFF9F5]">
          ...
          <div className="max-w-lg mx-auto px-4 py-6">
            <PageHeader
              title="スタッフマニュアル作成"
              description="AIが徹底的にヒアリングして、最強のマニュアルを一緒に作ります"
              backHref="/"
            />
    ...
    <AuthGuard>
      <div className="flex flex-col h-screen bg-[#FFF9F5]">
        ...
        <div className="bg-white border-b border-[#EDE5DF] px-4 py-3 flex items-center gap-3">
          ...
          <div className="w-8 h-8 rounded-full bg-[#E8320A] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
            AI
          </div>
          <div>
